...
@pytest.mark.asyncio
async def test_security_dashboard_does_not_claim_sandbox_active_for_dead_daemon(monkeypatch):
    """A Docker executable alone is not proof that isolated execution works."""
    ...
    monkeypatch.setattr("shutil.which", lambda _name: r"C:\\Program Files\\Docker\\docker.exe")

    async def daemon_unavailable(_self):
        return False

    monkeypatch.setattr(DockerPodmanBackend, "health_check", daemon_unavailable)
    async with get_db() as db:
        await db.execute("INSERT OR IGNORE INTO workspaces (id,name,description) VALUES (1,'Security QA','test')")
        await db.commit()
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.get(
            "/api/v1/security/status?workspace_id=1",
            headers={"Authorization": f"Bearer {TEST_OPERATOR_TOKEN}"},
        )

    assert response.status_code == 200
    sandbox = response.json()["docker_sandbox"]
    assert sandbox["status"] == "unavailable"
    assert sandbox["label"] == "Container Sandbox Unavailable"
    assert "daemon health check failed" in sandbox["evidence"]
